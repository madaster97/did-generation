...
module.exports = function (entityName) {
            ...
            keys.push(new Promise((res, rej) => {
                fs.readFile(path.join(entityPath, file), { encoding: 'utf-8' }, (err, data) => {
                    if (err) {
                        ...
                        logger("Private JWK read from file %s: %s", file, data)
                        res({
    ...
    return Promise.all(keys).then(privateKeys => {
        ...
        .then(async (json) => {
            ...
            console.log('Created did/doc for suffix %s. Saving...', didSuffix);
            ...
            console.log('Saved did/doc for suffix %s', didSuffix);
        });
}
